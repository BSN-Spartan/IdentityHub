...
public class HttpUtils<T> {
	...
	public static <T, E> ResultData<T> postCall(String url, String token, RequestParam<E> requestParam,
		...
		Request request = new Request.Builder().url(url).post(requestBody).header("Connection", "close").build();
		...
		try {
			...
			if (httpResonseCode != 200) {
				response.close();
				throw new HubException(ErrorMessage.UNKNOWN_ERROR.getCode(), ErrorMessage.UNKNOWN_ERROR.getMessage()+"service http code is "+httpResonseCode);
			}
		...
		if (resultCode != 0) {
			throw new HubException(resultCode, resultJson.getString("msg"));
		}
